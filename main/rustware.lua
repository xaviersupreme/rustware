--!strict
local UiSource: string = game:HttpGet("https://raw.githubusercontent.com/noowtf31-ui/Arcylic/refs/heads/main/src.lua.txt");
UiSource = string.gsub(UiSource, "self%._root%.Mesh%.Scale = Vector3%.new%(width, height, 0%)", "local _m = self._root:FindFirstChild('Mesh'); if _m then _m.Scale = Vector3.new(width, height, 0) end");
local Library = loadstring(UiSource)();
...
type CharacterController = {
    ...
    PerformRefuel: (self: CharacterController, GasPrompt: ProximityPrompt?, FuelPrompt: ProximityPrompt?) -> boolean
};
...
function UiManager.new(Title: string, Version: string): UiManager
    ...
    self.Window = Library.new(Title .. " " .. Version, "rustware_configs");
    return self;
end
...
function UiManager:Notify(Title: string, Text: string): nil
    self.Window:Notify({
        ...
        Description = Text,
        Duration = 3,
        Icon = "rbxassetid://7734056326"
    });
    return nil;
end

local CharController: CharacterController = CharacterController.new(LocalPlayer);
local Interface: UiManager = UiManager.new("rustware", "v6.0");


local MainSection: any = Interface.Window:CreateSection("General");
local MainTab: any = MainSection:CreateTab("Main", "rbxassetid://7733960981");
local TeleportsTab: any = MainSection:CreateTab("Teleports", "rbxassetid://7733992789");
local ClassesTab: any = MainSection:CreateTab("Classes", "rbxassetid://7743876054");
local MiscTab: any = MainSection:CreateTab("Misc", "rbxassetid://8997386997");
        ...
        if (Value) then 
            task.spawn(function()
                ...
                while (SpamTempEnabled) do 
                    pcall(function()
                        local TempLabel: TextLabel = Workspace.Plane.Machines.AirConditionner.Screen.SurfaceGui.Frame.TemperatureLabel;
                        local CurrentTemp: number? = tonumber(TempLabel.Text:match("%d+"));
    ...
    Callback = function(Value: boolean)
        ...
        if (Value) then
            local Success: boolean, ErrorMessage: string? = pcall(function()
                local RightBtnDetector: ClickDetector = Workspace.Plane.Machines.CenterConsole.RightButton.ClickDetector;
                local LeftBtnDetector: ClickDetector = Workspace.Plane.Machines.CenterConsole.LeftButton.ClickDetector;
                local ConditionFrame: Frame = Workspace.Plane.Machines.EngineConditionMachine.Screen2.SurfaceGui.Frame;
            ...
            if (not Success and ErrorMessage) then Interface:Notify("Error", ErrorMessage); end
        end
    end
});
...
MainTab:CreateToggle({
    ...
    Callback = function(Value: boolean)
        ...
        if (Value) then
            local DirectionText: TextLabel = Workspace.Plane.Machines.MissileDodgerMachine.Screen.SurfaceGui.Frame.Direction;
...
TeleportsTab:CreateSection("Jet Positions");
for Index = 1, 3 do
...
ClassesTab:CreateSection("Equip Class");
...
MiscTab:CreateSection("Points / Other");
...
Interface:Notify("Loaded", "Successfully loaded rustware v6.0");
